/// Source from the tokio project
///
/// This file implements a generic Blocking<T> type which can be used to add AsyncRead/AsyncWrite
/// impls to a struct which implements a blocking Read/Write interface.

use tokio::task::{spawn_blocking, JoinHandle};

use crate::exports::io::{AsyncRead, AsyncWrite};
        ...
        loop {
            match self.state {
                Idle(ref mut buf_cell) => {
                    ...
                    self.state = Busy(spawn_blocking(move || {
...
impl Buf {
    ...
    pub fn read_from<T: Read>(&mut self, rd: &mut T) -> io::Result<usize> {
    ...
    pub fn write_to<T: Write>(&mut self, wr: &mut T) -> io::Result<()> {
        assert_eq!(self.pos, 0);
...
impl Buf {
    pub fn discard_read(&mut self) -> i64 {
